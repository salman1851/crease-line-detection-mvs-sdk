...
class CameraOperation():
    ...
    # Add this method to your CamOperation_class
    def Start_grabbing_without_ui(self):
        self.cam.MV_CC_SetEnumValue("TriggerMode", MV_TRIGGER_MODE_OFF)
        ret = self.cam.MV_CC_SetEnumValue("PixelFormat", PixelType_Gvsp_Mono8)
        if ret != 0:
            print("Failed to set pixel format. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_SetEnumValue("AcquisitionMode", MV_ACQ_MODE_CONTINUOUS)
        if ret != 0:
            print("Failed to set acquisition mode. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_SetEnumValue("GainAuto", MV_GAIN_MODE_OFF)
        if ret != 0:
            print("Failed to set gain mode. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_SetFloatValue("Gain", 5)
        if ret != 0:
            print("Failed to set gain value. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_SetFloatValue("ExposureTime", 20000)
        if ret != 0:
            print("Failed to set exposure time. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_SetEnumValue("FrameRate", MV_FRAME_RATE_CTRL_OFF)
        if ret != 0:
            print("Failed to set frame rate. ret: [0x%x]" % ret)
            sys.exit()

        ret = self.cam.MV_CC_StartGrabbing()
        if ret != 0:
            print("Failed to start grabbing. ret: [0x%x]" % ret)
            sys.exit()
    ...
    def Work_thread(self,root,panel):
        ...
        while True:
            ...
            self.current_frame = np.array(current_image)  # Update the current_frame attribute

            img = Image.fromarray(np.array(current_image))
    ...
    def Save_jpg(self,buf_cache):
        ...
        file_path = 'image' + ".jpg"
        # print('save image file path: ', file_path)
        self.n_save_image_size = self.st_frame_info.nWidth * self.st_frame_info.nHeight * 3 + 2048
